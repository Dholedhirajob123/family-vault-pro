...
function SearchPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return (docs as any[]).filter((d) => {
      ...
      if (!s) return true;
      return [d.document_name, d.file_name, d.keywords, d.description]
  ...
  return (
    <div className="space-y-6">
      <Card className="glass rounded-3xl border-0 p-6">
        ...
        <p className="text-sm text-muted-foreground">Search across all family members by name, keyword or file.</p>
      ...
      <Card className="glass rounded-3xl border-0 p-2 md:p-4">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                <TableRow><TableCell colSpan={5} className="py-10 text-center text-muted-foreground">No matches.</TableCell></TableRow>
